...
import { createTaskSchema, getTaskSchema, updateTaskSchema, deleteTaskSchema } from "../schemas/taskSchema.js";
...
// GET one task by ID
taskRouter.get("/get-one/:id", schemaValidator(getTaskSchema, "params"), async (req, res) => {
    try {
        const { id } = req.params;
...
// CREATE one task
taskRouter.post("/create-one", schemaValidator(createTaskSchema, "body"), async (req, res) => {
    try {
...
// UPDATE one task
taskRouter.patch("/update-one/", schemaValidator(updateTaskSchema, "body"), async (req, res) => {
    try {
        const newTaskData = req.body;
...
// DELETE one task
taskRouter.delete("/delete-one/:id", schemaValidator(deleteTaskSchema, "params"), async (req, res) => {
    try {
        const { id } = req.params;
